...
openssl pkcs12 -export -out identity.p12 -inkey key.pem -in cert.pem -name "Txt Xpander Dev" \
  -certpbe PBE-SHA1-3DES -keypbe PBE-SHA1-3DES -macalg sha1 -passout pass:example-password
security import identity.p12 -k ~/Library/Keychains/login.keychain-db -P example-password -T /usr/bin/codesign
