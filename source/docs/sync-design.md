...
In practice the static-vs-dynamic case is nearly unreachable, and for a reason worth recording: `merge_snippets` is `{**static, **dynamic}`, so a static snippet sharing a name with a dynamic trigger is overwritten in memory, and the next `save_snippets` — which writes `build_saveable_snippets(self.snippets)` — **deletes it from `snippets.json`**. That is a pre-existing data-loss bug, out of scope for #31 and worth its own issue. The export's dedupe is therefore mostly defensive; implement it anyway, since #31 receives the static dict directly and a hand-edited file can still produce the case.
