"""Import ``txt_xpander.pyw`` as a module on every OS.

``importlib.machinery.SOURCE_SUFFIXES`` contains ``.pyw`` only on Windows, so
the plain ``import txt_xpander`` the tests used to do raises
``ModuleNotFoundError`` on macOS and Linux before a single test runs. Loading
the file explicitly by path keeps the modules that exercise ``TextExpander``
running on the whole CI matrix without renaming the app entry point (the
...
def _load():
    ...
    # Register before exec so that anything importing ``txt_xpander`` during
