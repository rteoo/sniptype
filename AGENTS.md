...
- Watch for stale project metadata: the README contains absolute links from another Codex worktree, `Txt Xpander.spec` hardcodes the original `C:\path\to\txt_xpander` paths, and the `txt_xpander.pyw` header still says version 2.6 while the changelog and README describe 2.7-era behavior.
