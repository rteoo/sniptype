...
Txt Xpander is a Windows system tray text expander. Typing a trigger word followed by a terminator such as a space or punctuation replaces the typed text with an expanded value, usually by placing the payload on the clipboard and sending Ctrl+V.
...
- `source\docs\` contains planning notes for refactors and features.
...
The keyboard hot path is `TextExpander.on_press()`. It appends keystrokes to a buffer, detects trigger terminators, and checks the compiled trigger index. Slow work such as network calls, dialogs, and form fill prompts must run outside the hot path.
...
- Watch for stale project metadata: the README contains absolute links from another Codex worktree, `Txt Xpander.spec` hardcodes the original `D:\OneDrive\_Dev\02.Python\txt_xpander` paths, and the `txt_xpander.pyw` header still says version 2.6 while the changelog and README describe 2.7-era behavior.
- The README mentions `run_txt_xpander.bat install`, but the inspected launcher does not currently implement an `install` branch.
