...
That last point is load-bearing and cost a process abort to learn (issue #53). Clicking the status item makes AppKit run a nested **menu-tracking loop** on the main thread, and Tk-Aqua's notifier rides the same run loop, so a pending Tcl timer fires *inside* it and calls back into Python while `_tkinter`'s `tcl_tstate` is NULL — `Fatal Python error: PyEval_RestoreThread`, SIGABRT, no traceback. Three rules follow, all with tests: `platform_support.observe_menu_tracking()` pauses the pump for the duration of a tracking session (synchronous `queue=None` delivery — an async observer can be scheduled after the timer it exists to prevent); `submit()` in main-thread mode **never** runs inline, because a tray callback building a window inside the tracking loop hits the same abort from the other direction (`call()` cannot do this — its caller blocks, which on the GUI thread would deadlock); and `stop()` from the GUI thread queues the teardown instead of using `root.after`, for the same reason. The pump stops draining as soon as the teardown runs, so nothing executes against a destroyed root.
