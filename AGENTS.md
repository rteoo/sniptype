...
plain `vMAJOR.MINOR.PATCH` tag (`v1.1.0`); current source is `1.1.0` on the
`stable` channel. The Microsoft Store listing is updated separately. The macOS ARM64
...
- `source\i18n.py` owns the interface language (`language` setting, Configurações > Geral: `pt-BR` or `en-US`; unset means follow the Windows display language, English → `en-US`, anything else → `pt-BR`, and nothing is persisted until the user chooses). `tests\app_module.py` pins `i18n.system_language` to Portuguese so an English host or CI runner does not flip the app's asserted text. Portuguese is the source language and each Portuguese string is its own message id: wrap every user-facing string in `_()` at display time, module-level constants in `N_()` (translated with `_()` where shown), tray labels in `lazy()`, and write `_("… {name} …").format(name=…)` instead of an f-string. English lives in `source\i18n_en_us.py`. `tests\test_i18n.py` reads the source and fails on unmarked Portuguese, a missing or unused English entry, or a placeholder mismatch; mark text that is deliberately not interface (logs, snippet output, persisted data) with `# i18n: not ui`. `test_manager_gui_smoke` builds the manager in English and fails on any Portuguese it shows. Snippet output (`xhoje`, BCB/stock summaries, WhatsApp) stays Portuguese.
